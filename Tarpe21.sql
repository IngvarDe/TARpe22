...
-- SalesCountry veerus omaette veergudeks koos veergude SalesAmount
-- liitmisega
...
as PivotTable
-- põhjuseks on Id veeru olemasolu ProductSalesWithId tabelis,
-- mida võetakse arvesse pivot (pööramise) ja grupeerimsie järgi
...
go
select Id, FromAgentOrCountry, SalesAgentName
from
(
...
unpivot
(
	SalesAgentName for FromAgentOrCountry in (SalesAgent, SalesCountry)
)
as PivotTable
...
-- A - atomic e aatmolikus
-- C - consistent e järjepidevus
-- I - isolated e isoleeritus
-- D - durable e vastupidav

--- Atomic - kõik tehingud transactions on kas edukalt täidetud või
-- need lükatakse tagasi. Nt, mõlemad käsud peaksid alati õnnestuma.
-- Andmebaas teeb sellisel juhul: võtab esimese update tagasi ja veeretab 
-- selle algasendisse e taastab algsed andmed.

--- Consistent - kõik transactioni puudutavad andmed jäetakse loogiliselt 
-- järjepidevasse olekusse. Nt, kui laos on saadaval olevate esemete hulka
-- vähendatud, siis tabelis peab olema vastav kanne. Inventuur ei saa
-- lihtsalt kaduda.

--- Isolated - transmaction peab andemdi mõjutama, sekkumata teistesse
...
-- muudetakse. Enamik DB-sid kasutab tehingute isoleerimise säilitamiseks
-- lukustamist.

--- Durable - kui muudatus on tehtud, siis see on püsiv. Kui süsteemisviga 
-- või voolukatkestus ilmneb enne käskude komplekti valmimist, siis
-- tühistatakse need käsud ja andmed taastatkse algsesse olekusse.
-- Taastamine toimub peale süsteemi taaskäivitamist.


--- sub queries
-- kuidas teha tabel andmetest tühjaks???
...
-- subquerit saab panna subquerie sisse
-- need on alati sulgudes


---- suurte andmetega tabel testimiseks
...
set @UpperLimitForProductId = 200000
-- UnitPrice
